  ...
  for (auto& i : composite_instructions)
  {
    ...
    else if (isPlanInstruction(i))
    {
      ...
      if (isStateWaypoint(base_instruction->getWaypoint()))
      {
        ...
        move_instruction.setProfileOverrides(base_instruction->getProfileOverrides());
                                       ...
                                       const tesseract_environment::Environment& env)
{
  ...
  std::string profile;
  std::unordered_map<std::string, std::shared_ptr<const void>> profile_overrides;
  if (isPlanInstruction(seed.getStartInstruction()))
  {
    ...
    profile_overrides = pi->getProfileOverrides();
  }
  ...
  if (isStateWaypoint(wp))
  {
    ...
    move_instruction.setProfileOverrides(profile_overrides);
