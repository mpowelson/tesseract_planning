...
int TimeOptimalTrajectoryGenerationTaskGenerator::conditionalProcess(TaskInput input, std::size_t unique_id) const
{
  ...
  std::vector<double> scaling_factors(flattened.size(), velocity_scaling_factor);
  ...
  {
    CompositeInstruction unflattened = unflatten(resampled, *ci, cur_composite_profile->path_tolerance);
                                                        ...
                                                        double tolerance) const
{
  ...
  unflattened[0].cast<CompositeInstruction>()->clear();
