...
int FixStateBoundsTaskGenerator::conditionalProcess(TaskInput input, std::size_t unique_id) const
{
  ...
  auto cur_composite_profile = getProfile<FixStateBoundsProfile>(
      profile, composite_profiles, std::make_shared<FixStateBoundsProfile>(), ci->getProfileOverride(name_));
